...
elseif($mode === 'CHEK_DATE')
{
    $START_WORK = trim(isset($_POST['START_WORK']) ? $_POST['START_WORK'] : '');
    $END_WORK = trim(isset($_POST['END_WORK']) ? $_POST['END_WORK'] : '');
    $GROUP_ID = (int)isset($_POST['GROUP_ID']) ? $_POST['GROUP_ID'] : '';
    $checkTime=new \local\Services\ResourcePlan\checkTime();
    $resultTime=$checkTime->checkStartWork($GROUP_ID,$START_WORK,$END_WORK);
    $resultTime["QUANTITY"]=$checkTime->getQuantity($resultTime["DATE_START"]["VALUE"],$resultTime["DATE_FINISH"]["VALUE"]);
    __CrmPropductRowListEndResponse($resultTime);
}
elseif($mode === 'GET_MODEL')
{
    $PRODUCT_ID = (int)isset($_POST['PRODUCT_ID']) ? $_POST['PRODUCT_ID'] : '';
    $Model=new \local\Services\ResourcePlan\Model();
    $obModel=$Model->getList($PRODUCT_ID);
    __CrmPropductRowListEndResponse(array('MODEL' => $obModel));
}
