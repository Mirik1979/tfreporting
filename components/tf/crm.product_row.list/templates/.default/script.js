...
if (typeof(BX.CrmProduct) === "undefined")
{
	...
	BX.CrmProduct.prototype =
	{
        ...
        addOption: function (oListbox, text, value, isDefaultSelected, isSelected)
        {
		...
		setSelectList(model,data){
            var self = this,
            objSel = document.getElementById(model);
            self.clearSelect(objSel);
            self.addOption(objSel,"Выберите продукт","");
            data.forEach(function(item, i, arr) {
                self.addOption(objSel,item.NAME,item.ID);
            });
		},
		setFieldView: function(fieldName, value)
		{
			...
			switch(fieldName)
			{
                        ...
                        BX.ajax(
                            {
                                ...
                                'data':
                                    {
                                        'OWNER_TYPE': this._editor.getSetting('ownerType', ''),
                                        'OWNER_ID': this._editor.getSetting('ownerID', 0),
                                        'MODE' : 'GET_MODEL',
                                        'PRODUCT_ID': parseInt(value),
                                        'SITE_ID': this._editor.getSetting('siteId', '')
                                    },
                                onsuccess: function(data)
                                {
                                    self.setSelectList(row.id + "_" + 'MODEL',data.MODEL);
                                },
                                onfailure: function(data)
                                {
                                    self._processAjaxError(data)
                ...
                case "START_WORK":
                case "END_WORK":
                	var StartWork=BX(row.id + "_" + "START_WORK"),
                        EndWork=BX(row.id + "_" + "END_WORK");
                    var data=
                        {
                            'OWNER_TYPE': this._editor.getSetting('ownerType', ''),
                            'OWNER_ID': this._editor.getSetting('ownerID', 0),
                            'GROUP_ID': this._editor.getSetting('GroupId', 0),
                            'MODE' : 'CHEK_DATE',
                            'START_WORK' : StartWork.value,
                            'END_WORK' : EndWork.value,
                            'SITE_ID': this._editor.getSetting('siteId', '')
                        };
                    var self = this;
                    BX.ajax(
                        {
                            ...
                            onsuccess: function(data)
                            {
                                ...
                                if(data.DATE_START.CHANGE=='Y')
                                    StartWork.value=data.DATE_START.VALUE;

                                if(data.DATE_FINISH.CHANGE=='Y')
                                    EndWork.value=data.DATE_FINISH.VALUE;

                                if(data.QUANTITY)
								{
                                    BX(row.id + "_" + "QUANTITY").value=data.QUANTITY;
                                    BX(row.id + "_" + "QUANTITY").dispatchEvent(new Event('keyup'))
								}
                            },
                            onfailure: function(data)
                            {
		...
		layout: function()
		{
            ...
            //this.setFieldValue("ROLE_PROJECT", this.getSetting('ROLE_PROJECT', 0));
		...
		saveSettings: function()
		{
            ...
            if(typeof (this._fieldValue['ROLE_PROJECT'])!=='undefined')
            	this._settings['ROLE_PROJECT'] = this._parseInt(this._fieldValue['ROLE_PROJECT'], 0);

            if(typeof (this._fieldValue['PRODUCT'])!=='undefined')
            	this._settings['PRODUCT'] = this._parseInt(this._fieldValue['PRODUCT'], 0);

            if(typeof (this._fieldValue['MODEL'])!=='undefined')
            	this._settings['MODEL'] = this._parseInt(this._fieldValue['MODEL'], 0);

            if(typeof (this._fieldValue['GRADE'])!=='undefined')
            	this._settings['GRADE'] = this._parseInt(this._fieldValue['GRADE'], 0);

            if(typeof (this._fieldValue['YEARS_EXPERIENCE'])!=='undefined')
            	this._settings['YEARS_EXPERIENCE'] = this._parseInt(this._fieldValue['YEARS_EXPERIENCE'], 0);
            ...
            if(typeof (this._fieldValue['START_WORK'])!=='undefined')
            	this._settings['START_WORK'] = BX.prop.getString(this._fieldValue, 'START_WORK');

            if(typeof (this._fieldValue['END_WORK'])!=='undefined')
            	this._settings['END_WORK'] = BX.prop.getString(this._fieldValue, 'END_WORK');
		...
		setSettings: function(settings)
		{
            ...
            /*this._settings['ROLE_PROJECT'] = parseInt(this.getSetting('ROLE_PROJECT', 0));
            this._settings['PRODUCT'] = parseInt(this.getSetting('PRODUCT', 0));
            this._settings['MODEL'] = parseInt(this.getSetting('MODEL', 0));
            this._settings['GRADE'] = parseInt(this.getSetting('GRADE', 0));
            this._settings['YEARS_EXPERIENCE'] = parseInt(this.getSetting('YEARS_EXPERIENCE', 0));
            this._settings['PREFERRED_LOCATION'] = parseInt(this.getSetting('PREFERRED_LOCATION', 0));
            this._settings['AMOUNT_OVERHEAD'] = this._round(parseFloat(this.getSetting('AMOUNT_OVERHEAD', 0.0)), 4);
            this._settings['START_WORK'] = this.getSetting('START_WORK', '');
            this._settings['END_WORK'] = this.getSetting('END_WORK', '');*/
		...
		toJson: function()
		{
			...
			if(this.getSetting("ROLE_PROJECT", false)!==false)
	            jsonData['ROLE_PROJECT'] =  this.getSetting("ROLE_PROJECT", 0);
            if(this.getSetting("PRODUCT", false)!==false)
            	jsonData['PRODUCT'] =  this.getSetting("PRODUCT", 0);
            if(this.getSetting("MODEL", false)!==false)
            	jsonData['MODEL'] =  this.getSetting("MODEL", 0);
            if(this.getSetting("GRADE", false)!==false)
            ...
            if(this.getSetting("END_WORK", false)!==false)
            	jsonData['END_WORK'] = this.getSetting("END_WORK", "");
...
$(document).ready(function () {
	...
	$(document).on('click','#import_from_excel',function (e) {
        e.preventDefault();
        var file_data = $('#import_from_excel_file').prop('files');
        ...
        if(file_data.length<=0)
		{
            ...
            var addloader = new BX.PopupWindow(
                "my_loader",
                null,
                {
                    overlay: {
            ...
            $.ajax({
                url: '/local/components/tf/crm.product_row.list/file.php',
                dataType: 'json',
                cache: false,
                contentType: false,
                processData: false,
                ...
                success: function(data){
                    addloader.close();
                    if(data.error){
                        var errorloader = new BX.PopupWindow(
                            "my_error",
                            null,
                            {
                                overlay: {
