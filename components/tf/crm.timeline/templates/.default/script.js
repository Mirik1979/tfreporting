...
if(typeof(BX.CrmHistoryItem) === "undefined")
{
	...
	BX.CrmHistoryItem.prototype.prepareHeaderLayout = function()
	{
        ...
        header.appendChild(getStatusBlock(this));
		return header;
	};
...
function getStatusBlock(_this){
    var el=BX.create("SPAN",
        {
            attrs: {
                id: "crm-entity-stream-content-event-status-"+_this._id,
                className: "crm-entity-stream-content-event-status"
            },
            text: ''
        }
    );
    var form_data = new FormData();
    form_data.append('id', _this._id);
    form_data.append('sessid', BX.bitrix_sessid());
    $.ajax({
        url: '/ajax/getStatusBlock.php',
        dataType: 'json',
        cache: false,
        contentType: false,
        processData: false,
        ...
        success: function(data){
            if(data.name)
                $("#crm-entity-stream-content-event-status-"+_this._id).text(data.name);
        }
    });
    return el;
}
