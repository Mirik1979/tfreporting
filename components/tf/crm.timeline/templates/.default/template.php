			...
			<div class="crm-entity-stream-filter-container">
				...
				$APPLICATION->includeComponent(
					...
					array(
						...
						'LAZY_LOAD' => array(
							'GET_LIST' => '/ajax/crm.timeline/filter.ajax.php?action=list&filter_id='.urlencode($arResult['HISTORY_FILTER_ID']).'&siteID='.SITE_ID.'&'.bitrix_sessid_get(),
							'GET_FIELD' => '/ajax/crm.timeline/filter.ajax.php?action=field&filter_id='.urlencode($arResult['HISTORY_FILTER_ID']).'&siteID='.SITE_ID.'&'.bitrix_sessid_get(),
						)
					)
				);
				?>
		...
		function()
		{
			...
			BX.CrmTimelineManager.create(
				"<?=CUtil::JSEscape($guid)?>",
				{
					...
					serviceUrl: "/ajax/crm.timeline/ajax.php?&site=<?=SITE_ID?>&<?=bitrix_sessid_get()?>",
